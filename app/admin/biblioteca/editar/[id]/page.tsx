...
export default function EditBook() {
    ...
    const fetchBook = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          setErrorMessage("No se encontró el libro.");
        }
      } catch (error) {
  ...
  if (authLoading || isLoadingData) return <div className="min-h-screen flex items-center justify-center">Cargando datos...</div>;
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="pt-32 pb-24 px-6 max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-12">
          ...
          <div>
            <h1 className="text-3xl font-bold serif">Editar Libro</h1>
            <p className="text-muted-foreground text-sm">Modifica los detalles del ejemplar.</p>
          </div>
        </div>
          ...
          >
            {isSubmitting ? (
              <>
                ...
                {uploadProgress !== null ? `Subiendo portada... ${Math.round(uploadProgress)}%` : "Guardando cambios..."}
              </>
            ) : (
              <>
